---
name: o-timeline
description: This skill should be used when the user request matches the former assistant workflow `o-timeline`. [Organize] 时间线视图 - 混合展示手动记录与自动追踪
---

# Codex 适配说明

- 这是原 Claude assistant 命令 `o-timeline` 的 Codex skill 版本。
...
展示指定日期的时间线，混合显示手动间隙记录和 Codex 活动数据。
...
```bash
python3 "${ASSISTANT_PLUGIN_ROOT:-$HOME/plugins/assistant}/scripts/analyze-codex-activity.py" 用户提供的参数 --json-only
```
