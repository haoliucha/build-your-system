---
name: o-review
description: This skill should be used when the user request matches the former assistant workflow `o-review`. [Organize] 每日回顾 - 分发 + 复盘 + 规划
---

# Codex 适配说明

- 这是原 Claude assistant 命令 `o-review` 的 Codex skill 版本。
...
上面是当日 Codex 活动数据（时间线、时间投入、领域分布）。Phase 2 复盘时使用。
...
## Phase 2: 复盘（GRAI + 4Fs 融合 + Interstitial Journaling）

### 2.0 展示时间线（新增）

**读取间隙日志**：
1. 读取 `00-Inbox/{日期}.md` 的 `## 间隙日志` 区块
2. 解析每个 `### {HH:MM}` 条目
3. 提取：时间、完成、感受、下一步
...
|-------|------------------|-----------------------------|-----------------------------|
| 12:04 | 复刻系统教程     | 20 | 启动项目规划                | Obsidian + Codex 教程 |
