...
//      MAX_SCROLL_ROUNDS=160, SCROLL_WAIT_MS>=8000, SCROLL_IDLE_LIMIT=8,
...
const { coverageSummary } = require(path.join(__dirname, 'lib', 'following-diff.cjs'));
const { assertRunToken } = require(path.join(__dirname, 'lib', 'rate-gate.cjs'));
const {
  SNAPSHOT_MAX_ROUNDS,
  SNAPSHOT_WAIT_MIN_MS,
  SNAPSHOT_WAIT_MAX_MS,
  SNAPSHOT_LONG_BREAK_EVERY,
  SNAPSHOT_LONG_BREAK_MS,
  boundedInt,
  jitterMs,
...
const MAX_SCROLL_ROUNDS = boundedInt(process.env.MAX_SCROLL_ROUNDS, SNAPSHOT_MAX_ROUNDS, { min: 1, max: SNAPSHOT_MAX_ROUNDS });
const SCROLL_WAIT_MIN_MS = boundedInt(process.env.SCROLL_WAIT_MS, SNAPSHOT_WAIT_MIN_MS, { min: SNAPSHOT_WAIT_MIN_MS });
const SCROLL_WAIT_MAX_MS = boundedInt(process.env.SCROLL_WAIT_MAX_MS, SNAPSHOT_WAIT_MAX_MS, { min: SCROLL_WAIT_MIN_MS });
const LONG_BREAK_EVERY = boundedInt(process.env.SNAPSHOT_LONG_BREAK_EVERY, SNAPSHOT_LONG_BREAK_EVERY, { min: 1, max: SNAPSHOT_LONG_BREAK_EVERY });
const LONG_BREAK_MS = boundedInt(process.env.SNAPSHOT_LONG_BREAK_MS, SNAPSHOT_LONG_BREAK_MS, { min: SNAPSHOT_LONG_BREAK_MS });
...
async function main() {
  assertRunToken();
  ensureDir(SNAP_DIR);
  say(`@${HANDLE} /following  date=${DATE}  rounds<=${MAX_SCROLL_ROUNDS} idle=${SCROLL_IDLE_LIMIT} wait=${SCROLL_WAIT_MIN_MS}-${SCROLL_WAIT_MAX_MS}ms long-break=${LONG_BREAK_MS}ms/${LONG_BREAK_EVERY} rounds`);
  ...
  for (rounds = 1; rounds <= MAX_SCROLL_ROUNDS; rounds++) {
    ...
    if (idle >= SCROLL_IDLE_LIMIT) {
      ...
      if (coverageFloor && scanned.size < coverageFloor && recoveries < 2) {
        ...
        await page.waitForTimeout(SCROLL_WAIT_MIN_MS);
        for (let i = 0; i < 5; i++) {
    ...
    await page.evaluate(() => window.scrollBy(0, window.innerHeight * 0.85));
    const wait = jitterMs(SCROLL_WAIT_MIN_MS, SCROLL_WAIT_MAX_MS);
    await page.waitForTimeout(wait);
    if (rounds % LONG_BREAK_EVERY === 0) {
      say(`rate-safety long break ${Math.ceil(LONG_BREAK_MS / 1000)}s after round ${rounds}`);
      await page.waitForTimeout(LONG_BREAK_MS);
    }
  }
  { // final sweep
  ...
  const coverage = coverageSummary(scannedTotal, headerFollowingCount, MIN_COVERAGE_PCT);
  const { rawCoveragePct, coveragePct, coverageWarning } = coverage;
  ...
  const metaFile = path.join(SNAP_DIR, `${DATE}.meta.json`);
  const meta = {
    date: DATE, handle: HANDLE, generatedAt: extractedAt, count: rows.length,
    mutualCount, nonRecipCount, rounds, recoveries, retryClicks,
    ...coverage,
  };
  fs.writeFileSync(metaFile, `${JSON.stringify(meta, null, 2)}\n`, 'utf8');

  say(`wrote ${rows.length} rows (${nonRecipCount} non-recip, ${mutualCount} mutual) -> ${file}`);
  say(`coverage: scanned=${scannedTotal} expected=${headerFollowingCount === null ? 'unknown' : headerFollowingCount} raw=${rawCoveragePct === null ? 'n/a' : rawCoveragePct}% display=${coveragePct === null ? 'n/a' : coveragePct}% rounds=${rounds} recoveries=${recoveries} retryClicks=${retryClicks}`);
  ...
  console.log(JSON.stringify({
    date: DATE, handle: HANDLE, count: rows.length, file, metaFile,
    scannedTotal, mutualCount, nonRecipCount, headerFollowingCount, rawCoveragePct, coveragePct, coverageWarning,
  }, null, 2));
