...
- meta、normalizer、facet schema/prompt 与 source fingerprint 共同决定 facet 缓存有效性；Lens prompt 与 Report schema 独立版本化。只改 Lens 或 UI 不使 facet 失效，语言变化也不使 facet 失效。
- 发布门禁产物只写 `previews/<version>/`，写入前后校验正式 report/state/manifest/facet 哈希；预览不得成为正式缓存或覆盖正式报告。
