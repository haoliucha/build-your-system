...
const skip = new Set(buildSkipSet([tracker])); // followed ∪ rejected
...
process.stderr.write(`[build-queue] files=${files.length} crypto=${NOCRYPTO ? 'filtered' : 'KEPT'} ${JSON.stringify(stats)}\n`);
