...
from helpers import STYLE_NAMES, X_IMAGE, read_optional


class StyleContractTests(unittest.TestCase):
    def setUp(self):
        self.policy = read_optional(X_IMAGE / "references" / "style-policy.md")
        self.styles = {
            name: read_optional(X_IMAGE / "styles" / name)
    ...
    def test_terminal_motifs_cannot_create_extra_glyphs(self):
        ...
        self.assertIn("Terminal motifs must use abstract geometry only.", terminal)
        for phrase in (
            "cursor glyphs",
            "prompt symbols",
            "code characters",
