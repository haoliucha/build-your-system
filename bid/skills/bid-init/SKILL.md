...
本节仅适用于全新成单项目或经确认转正/重组后的正式项目；新线索分支不初始化 `.claude/memory/`。在项目 `.claude/memory/` 中按现有命名约定建立项目主题文件，并在其中的 `MEMORY.md` 索引增加一行：项目名、一句话摘要、核心口径占位。已有主题文件或索引不得覆盖；先展示拟新增内容或 diff，等待确认。这个索引是后续会议工作流（Claude `/bid:meeting`、Codex `$bid:bid-meeting`）与数字同步工作流（Claude `/bid:sync`、Codex `$bid:bid-sync`）落入口径决策、防止旧口径回潮的位置。
...
- 下一步：第一次会议用 Claude `/bid:meeting` 或 Codex `$bid:bid-meeting`；修改任何对客数字用 Claude `/bid:sync` 或 Codex `$bid:bid-sync`；对外材料出稿前使用 `adversarial-review`；
