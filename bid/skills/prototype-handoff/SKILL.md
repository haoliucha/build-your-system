...
```bash
bash scripts/extract-frames.sh frames <录屏.mp4> <帧目录> 3   # 固定节奏抽帧
bash scripts/extract-frames.sh sheet <帧目录> <sheet目录>     # 编号 contact sheet
bash scripts/extract-frames.sh pixel <帧.jpg> <x> <y>         # 像素 hex 取样
```
