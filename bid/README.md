...
脚本会先验证 marketplace 名称、精确 `bid` 条目和 `~/plugins/bid` 的实际目标。`codex plugin list --json` 依赖本地 marketplace 发现：仅删除 marketplace 中的 `bid` 条目，就可能让 list 漏掉仍保留在 Codex 配置与缓存中的孤儿安装，脚本不会把 list 的空结果当作已卸载证据。因此，本地条目和链接都缺失时，脚本会临时建立这两个精确登记，并把原 marketplace inode 保留在事务隔离区；随后先查询 list，执行幂等的 `codex plugin remove bid@local-build-your-system`，再查询 list 来验证 Codex 自有状态，只有后状态确认为 absent 才恢复原始的本地 absent 状态。remove 的退出码本身不作为成功证据；后状态仍为 installed 时最多再做一次有界 remove 并重新查询。若查询失败、JSON 合同异常或两次 remove 都没有改变后状态，脚本失败关闭；尚未执行 remove 时会精确恢复原 marketplace inode，已执行但无法证明 absent 时则保留临时登记和链接，让孤儿仍可发现并给出恢复诊断。本地登记仍精确完整时，list 用于记录操作前状态并验证操作后状态，随后仍总是执行 remove，不用操作前的空结果跳过清理。

remove 命令会删除 Codex 自己管理的已安装配置与缓存；这是卸载的预期行为。它不会删除源码 checkout、Claude 状态、项目内 `.claude/memory/`，也不会改动无关的 marketplace 条目。如果 Codex CLI 移除失败，marketplace 与链接保持原样；成功后脚本才移除单个 `bid` 条目和这个精确链接，保留其他 marketplace key、插件及顺序。marketplace 的存在性、文件类型、设备与 inode、权限、mtime、大小和字节必须与预检快照一致；即使只是 chmod 或用相同字节替换了文件，脚本也会停止。最终写入前，脚本会用 macOS 的 no-replace rename 把当前路径移交到随机隔离区，在隔离区再次验证 inode、权限和字节，再用 no-replace hard link 建立新文件；源码链接的移除也先做同样的所有权移交。生命周期不会对隔离路径执行存在 check 后的 unlink，因为同用户进程仍可能在 check 与 unlink 之间替换对象；验证后的 marketplace、source 和临时写入对象会作为恢复产物保留。脚本在成功、失败或 SIGINT/SIGTERM 退出前都会逐条输出仍存在的精确 `Recovery artifact retained: <path>`，包括孤儿移除失败时的 `lease.original_quarantine`。只有人工确认该随机路径仍是本次报告的产物后才可另行清理。若外部进程在最后检查与动作之间写入原路径或隔离路径，外部对象不会被覆盖或 unlink；无法无覆盖恢复时，错误会保留隔离恢复产物并报告其路径。回滚与恢复使用相同的 no-clobber 合同，SIGTERM 仍保持 143 退出语义。
